...
namespace Стамотология
{
    public partial class MainForm : Form
    {
        ...
        {            
            ...
            string queryTest = "select * from Льготник";

            DataTable dtTest = ДанныеПредставление.GetПредставление(query, "ЛьготникСписок");

            //try
            //{
            //    string sTestQuery = queryTest;

            //    DataSet ds = new DataSet();
            //    OleDbConnection con = new OleDbConnection(ConnectionDB.ConnectionString());
            //    con.Open();
            //    OleDbDataAdapter da = new OleDbDataAdapter(queryTest, con);


            //    da.Fill(ds, "ЛьготникСписок");
            //    con.Close();
        ...
        private void открытьYandexДискToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Process ieProcess = Process.Start("https://yadi.sk/d/I8we1-c2vER7Yg?w=1");
        }
    }
}